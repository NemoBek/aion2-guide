...
const { routeSteps, after45, classData, farmTiers, mechanics } = guide;
const allTaskIds = [...routeSteps.map((item) => item.id), ...after45.map((item) => item.id)];

const navItems = [["start", "Старт", Compass], ["route", "1–45", Route], ["endgame", "После 45", Crown], ["farm", "Фарм", Gem], ["mechanics", "Механики", Hammer], ["classes", "Классы", Swords]] as const;
        ...
        <footer className="px-4 py-10 sm:px-8 xl:px-14">
          <div className="flex flex-col justify-between gap-6 rounded-2xl border border-white/8 bg-white/[0.025] p-5 sm:flex-row sm:items-start">
            <div>
              ...
              <p className="mt-4 text-xs text-slate-500">Последняя успешная синхронизация: {guide.lastSyncedAt ? guide.lastSyncedAt.slice(0, 10).split("-").reverse().join(".") : "ещё не выполнялась"}</p>
            </div>
